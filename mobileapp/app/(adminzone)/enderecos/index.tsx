...
import { ActivityIndicator, FlatList, RefreshControl, Text, View, Modal, TouchableOpacity } from 'react-native';
		...
		<ThemedView className="relative flex-1 w-full p-5">
			<ThemedView className="flex flex-col flex-1 w-full gap-4">
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<View className="w-full p-4 flex gap-4 !bg-slate-100 dark:!bg-slate-800 rounded-lg">
							<View className="flex flex-1 gap-4">
								<Text className="font-semibold text-slate-600 dark:text-slate-100">{item.cep} - {item.logradouro}, {item.numero}</Text>
								<Text className="uppercase text-slate-600 dark:text-slate-100">{item.cidade} - {item.estado}</Text>
							</View>
							<View className="grid grid-cols-2 gap-4">
								<Button color="primary" className="flex flex-row w-full gap-2 h-[30px]" onPress={() => { }}>
									<Ionicons name="pencil-outline" color={'#ffffff'} size={16} />
									<Text className="text-white uppercase">Editar</Text>
								</Button>

								<Button color="danger" className="flex flex-row w-full gap-2 h-[30px]" onPress={() => openDeleteModal(item)}>
									<Ionicons name="trash-outline" color={'#ffffff'} size={16} />
									<Text className="text-white uppercase ">Remover</Text>
								</Button>
							</View>
						</View>
					)}
